<p><h1 align="center" style="color:maroon"><img src="src/main/resources/assets/gilding/textures/gui/title/gilding.png" width="400"> <br>
<p><h1 align="center" style="color:gold"> CoreAPI <br>
</h1>
<br>
<br>
<h2 align="center">
<a href="https://fabricmc.net"><img src="https://cdn.discordapp.com/attachments/705864145169416313/969720133998239794/fabric_supported.png" width="200"></a> <br>
<a href="https://github.com/Illyena/Gilding-core-Api/blob/1.20.1-dev/LICENSE"><img src="https:shields.io/github/license/Illyena/Gilding-core-Api"></a>
</h2>

## Description:
Private dependencies for Gilding's core mod.
### Authors:
[Illyena](https://github.com/Illyena)

## Setup:
![Mod loader: Fabric](https://img.shields.io/badge/modloader-Fabric-1976d2?style=flat-square&logo=data:image/png;base64,)
![enviornment](https:img.shields.io/badge/environment-server-4caf50)

Please install an instance of Fabric Loader for your client (and the server if playing multiplayer).
### Dependencies:
<p><a href="https://modrinth.com/mod/fabric-api"><img src="https://i.imgur.com/Ol1Tcf8.png" width="200"></a></p>
This mod is required for the following mods and all of their dependents:

-[Gilding-core](https://modrinth.com/mod/Gilding-core)




### Please report any issues to the Github repository
[![issues](https://shields.io/github/issues/Illyena/Gilding-core-Api)](https://github.com/Illyena/Gilding-core-Api/issues)

### For Developers:
[![jitpack](https://jitpack.io/v/Illyena/Gilding-core-Api.svg)](https://jitpack.io/#Illyena/Gilding-core-Api) <br>
add the following to your `build.gradle` file
```gradle
repositories {
    maven { url 'https://jitpack.io' }
}
dependencies {
   modImplementation "com.github.Illyena:Gilding-core-Api-mc${project.minecraft_version}:${project.core_api_version}"
}
```

add the following to your `gradle.properties` file
```gradle
#Dependencies
    core_api_version=0.0.1
```


### License
This mod is available under the [![GitHub](https://img.shields.io/github/license/Illyena/Gilding-core-Api)](https://github.com/Illyena/Gilding-core-Api/blob/1.18.2/LICENSE)
